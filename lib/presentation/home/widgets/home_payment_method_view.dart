...
class HomePaymentMethodView extends StatelessWidget {
  final HomeState state;
  final TextEditingController controller;
  const HomePaymentMethodView({
    super.key,
    required this.state,
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      padding: EdgeInsets.symmetric(
        horizontal: 16.w,
      ).add(EdgeInsets.only(top: 5.h, bottom: 6.h)),
      child: AppTextFormField(
        controller: controller,
        labelText: 'home.paymentMethod.label'.tr(),
        hintText: 'home.paymentMethod.placeholder'.tr(),
        readOnly: true,
        showCursor: false,
        suffixIcon: Icon(Icons.keyboard_arrow_down_rounded, color: cs.tertiary),
        onTap: () {
          final list = state.homePaymentMethodDto?.data?.paymentModes ?? [];
          if (list.isEmpty) return;
          if (list.length == 1) {
            AppNotify.toast(
              'home.paymentMethod.onlyOneOption'.tr(
                namedArgs: {
                  'paymentMethod': list.first.name ?? '',
                },
              ),
          ...
          context.router.push(
            AppSelectableRoute(
              title: 'home.paymentMethod.label'.tr(),
              ...
              onSelected: (value) => context.read<HomeBloc>().add(
                HomeEvent.paymentMethodSelected(
                  selectedPaymentMethodItem: value,
                ),
              ),
            ),
          );
